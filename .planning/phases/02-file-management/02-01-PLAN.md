...
<task type="auto">
  ...
  <action>
    Install Tauri plugins via CLI (this updates both Cargo.toml and registers plugins in lib.rs):
    ...
    IMPORTANT: After each `npm run tauri add` command, verify it succeeded by checking exit code and output. The commands MUST complete successfully - if they fail, the entire plan fails.
  ...
  <verify>
    ...
    - `grep "tauri-plugin-dialog" src-tauri/Cargo.toml` shows version 2
    - `grep "tauri-plugin-store" src-tauri/Cargo.toml` shows version 2

    Verify plugins registered in lib.rs (CRITICAL - this confirms tauri add commands succeeded):
    - `grep -E "tauri_plugin_fs::init|tauri_plugin_dialog::init|tauri_plugin_store::Builder" src-tauri/src/lib.rs | wc -l` returns 3 (all 3 plugins registered)

    Verify npm packages installed:
    - `grep "@tauri-apps/plugin-fs" package.json` shows installed
    - `grep "@tauri-apps/plugin-dialog" package.json` shows installed
    - `grep "@tauri-apps/plugin-store" package.json` shows installed
    - `grep "react-hotkeys-hook" package.json` shows installed
    - `grep "use-debounce" package.json` shows installed
  </verify>
  <done>
    All Tauri plugins (fs, dialog, store) registered in lib.rs, dependencies in Cargo.toml, and npm packages (plugin bindings, react-hotkeys-hook, use-debounce) installed.
  </done>
</task>
