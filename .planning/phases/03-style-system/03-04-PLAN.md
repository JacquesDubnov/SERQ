...
Purpose: Connect the style panel, format painter, theme detection, and document persistence into a working system. Update editor CSS to use CSS variables so presets actually change the document appearance.
...
```
  ...
  <done>All editor typography and colors use CSS variables, changing a preset visually updates the editor</done>
</task>
...
```
  ...
  <done>App has style panel toggle, theme toggle, styles initialize on mount</done>
</task>
...
```
  ...
  <done>Opening documents loads their styles, saving documents persists current styles, new documents get user defaults</done>
</task>

<task type="auto">
  <name>Task 4: Wire Toolbar</name>
  <files>src/components/Editor/EditorToolbar.tsx</files>
  <action>
Add Format Painter to the toolbar.
...
**Add to toolbar (after alignment buttons):**
```tsx
<Divider />
...
Note: Cmd+Shift+S is Save As. Use different shortcut for style panel.
Options: Cmd+. (period), Cmd+/, F6
Recommendation: Cmd+Shift+Y (easy to remember, not commonly used)
  </action>
  <verify>Format painter button appears in toolbar. Click it, verify it activates. Keyboard shortcut opens style panel.</verify>
  <done>Format painter in toolbar, keyboard shortcut for style panel</done>
</task>
...
**Test 6: Persistence**
1. Apply a non-default style (e.g., Modern Sans + Sepia)
2. Save document (Cmd+S)
3. Close and reopen the document
4. Verify: Styles are restored

**Test 7: Style Panel UX**
...
<verification>
1. CSS variables control editor appearance
2. Style panel opens/closes with animation
3. Preset clicks update document instantly
4. Theme toggle switches light/dark
5. Format painter copies and applies formatting
6. File save/load preserves styles
...
- [ ] File operations load/save style metadata
- [ ] Style changes mark document dirty
- [ ] Human verification passes all 7 tests
</success_criteria>
