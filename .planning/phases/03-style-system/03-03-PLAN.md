...
must_haves:
  truths:
    ...
    - "Layout section shows layout presets (STYLE-03)"
    - "Master Themes section shows predefined combinations (STYLE-04)"
  ...
  artifacts:
    - path: "src/components/StylePanel/StylePanel.tsx"
      provides: "Main slide-in panel component with 5 accordion sections"
      exports: ["StylePanel"]
    - path: "src/components/StylePanel/MasterThemeSection.tsx"
      provides: "Master themes accordion section"
      exports: ["MasterThemeSection"]
...
Purpose: Users need a visual interface to browse and apply presets. The style panel slides in from the right with accordion sections for Master Themes, Typography, Colors, Canvas, and Layout. Format painter lets users copy formatting between selections.

Output: Working style panel component with all 5 sections, working format painter with toggle/hold modes
</objective>
...
<task type="auto">
  ...
  <files>src/components/StylePanel/StylePanel.tsx, src/components/StylePanel/PresetSection.tsx, src/components/StylePanel/PresetButton.tsx, src/components/StylePanel/MasterThemeSection.tsx, src/components/StylePanel/index.ts</files>
  <action>
...
**Sections (accordion - 5 total):**
1. **Master Themes** (collapsed by default) - 33 predefined combinations (STYLE-04)
2. **Typography** (expanded by default) - 23 presets
3. **Colors** - 25 presets
4. **Canvas** - 25 presets
5. **Layout** (NEW - STYLE-03) - 25 presets

**Footer:**
- "Save Current Style" button
- "Set as Default" button

**MasterThemeSection.tsx (NEW - for STYLE-04):**
```typescript
...
```
- Shows 33 master themes as larger cards (not just buttons)
- Each card shows: name + small preview of typography/color/canvas combo
- Clicking applies all four presets at once
- Current master theme highlighted (or "Custom" shown if mix doesn't match any theme)
- Organized by category: Writing (novelist, journalist, etc.), Dark Modes, Minimal, etc.
...
Export all from index.ts.
  ...
  <verify>Import StylePanel, render with isOpen=true. Panel should appear on right side with 5 accordion sections (Master Themes, Typography, Colors, Canvas, Layout).</verify>
  <done>Style panel slides in/out, has five accordion sections including Layout and Master Themes, presets are clickable buttons, recently used section works</done>
</task>
...
5. Layout section (STYLE-03) shows 25 layout presets
6. Master Themes section (STYLE-04) shows 33 predefined combinations
7. Format painter: select bold text, activate, select plain text, click. Plain text becomes bold.
8. Format painter toggle mode: stays active after applying, apply multiple times
</verification>

<success_criteria>
- [ ] StylePanel component renders and animates in/out
- [ ] Panel pushes content, doesn't overlay
- [ ] Accordion has 5 sections (Master Themes, Typography, Colors, Canvas, Layout)
- [ ] Clicking preset calls styleStore.setTypography/setColor/setCanvas/setLayout
- [ ] Clicking master theme calls styleStore.setMasterTheme (applies all 4 presets)
- [ ] Recently used section shows last 5 used presets per category
