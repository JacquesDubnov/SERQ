...
<task type="auto">
  ...
  <verify>
    1. In dev tools console: editor.commands.insertCallout({ color: 'blue' })
    2. Callout block appears with blue background
    3. Can type paragraph text inside it
    4. Type "### Heading" inside callout, verify H3 renders (confirms content: 'block+' works)
    5. Create a bullet list inside callout, verify list renders properly
  </verify>
  <done>
    Callout node extension created and renders with editable block content (paragraphs, headings, lists).
  </done>
</task>
