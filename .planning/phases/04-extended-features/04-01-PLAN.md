...
must_haves:
  ...
  key_links:
    ...
    - from: "EditorWrapper.tsx"
      to: "TableContextMenu"
      via: "contextmenu event listener"
      pattern: "onContextMenu|contextmenu.*TableContextMenu"
---
...
<task type="auto">
  ...
  <action>
    ...
    Wire context menu in EditorWrapper.tsx (NOT EditorCore - EditorWrapper handles UI overlays):
    - Add state: const [tableMenuState, setTableMenuState] = useState<{ x: number, y: number } | null>(null)
    - Add onContextMenu handler to the wrapper div around EditorCore
    - Check if right-click is inside a table cell: event.target.closest('td, th')
    - If inside table, show TableContextMenu at event coordinates
    - Pass editor ref to TableContextMenu
