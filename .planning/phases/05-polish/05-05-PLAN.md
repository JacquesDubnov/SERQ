...
must_haves:
  ...
  key_links:
    - from: "src/extensions/Comment/Comment.ts"
      ...
      via: "onCommentActivated callback updates store"
      pattern: "onCommentActivated.*commentId"
  ...
  notes:
    - "Comment extension is self-contained: click handling wires to store via onCommentActivated option"
    - "This plan creates all layers: SQLite, storage lib, Zustand store, TipTap extension, React panel"
    - "Plan 05-07 integrates by passing onCommentActivated callback when registering extension"
---
...
```
  ...
  <done>
    Comment TipTap extension created with setComment/unsetComment commands and self-contained click handling via onCommentActivated option
  </done>
</task>
...
- Click on comment mark activates comment via onCommentActivated callback (configured at registration)
